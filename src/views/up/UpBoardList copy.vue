<template>
    <div class="board-list">
      <div class="common-buttons">
        <button type="button"  class="btn btn-warning" v-on:click="fnWrite">등록</button>
      </div>
      <table class="table  table-hover">
        ...
        <tr v-for="(row, idx) in list" :key="idx">
          ...
          <!-- {{ row.imgn }}  -->  
           ...
           <img :src="`/images/${ row.imgn }`" style="width: 100px;"/>
  ...
  methods 안의 fnGetList를 수정합니다. */
      fnGetList() {
        ...
        this.$axios.get(this.$serverUrl + "/upboard/list", {
